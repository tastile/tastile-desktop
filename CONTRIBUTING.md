...
The validation script is strict by default. It runs formatting and dependency checks,
unit tests, both desktop builds (default and `win-x64`), and generated connector
safety checks for `TimelineWindow`. The desktop build is self-contained and does not
require a sibling `tastile-core` checkout.
