...
{
    public static DaemonManager Shared { get; } = new();
    ...
    public async Task<bool> EnsureRunningAsync()
    {
        ...
        try
        {
            ...
            // If we already started a child daemon, give that exact process a chance to come up
            // instead of treating it as stale during concurrent EnsureRunningAsync calls.
            ...
            DaemonCompatibility.KillStaleDaemonProcesses(
                Process.GetCurrentProcess().Id,
                _daemonProcess is { HasExited: false } ? _daemonProcess.Id : null);
