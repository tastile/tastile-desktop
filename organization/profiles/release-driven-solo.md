...
Tastile Desktop の current release-driven development で使用する Operating Model を定義します。
...
- `tastile-root` の cross-repo contract と `tastile-core` API/domain authority を shared project decision として尊重する。
- `global.json` / .NET / WinUI 3 / Windows host verification を project-local toolchain/evidence source として維持し、mise に競合 pin を作らない。
- installed desktop / installer verification は source-only test と分離し、current candidate identity へ bind する。
