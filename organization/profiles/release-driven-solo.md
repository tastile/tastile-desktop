...
- Windows native / WinUI / .NET SDK pin / host verification の既存 contract を保持する。
- Windows native verification は current candidate commit/artifact に bind し、WSL/Linux source checks と混同しない。
- Tastile domain/API authority は core/workspace contract を参照する。
...
- `skills-lock.json` は CLI-generated source/freshness metadata として commit する。
