...
public sealed class PromptActionDispatcherTests
{
    ...
    [Fact]
    public async Task ExecuteAsync_CompletePhase_UsesFallbackTileIdAndScope()
    {
        string? capturedPath = null;
        string? capturedBody = null;
        var api = CreateApiClient(request =>
    ...
    public async Task ExecuteAsync_StartupRecoveryAction_UsesStartupRecoveryEndpoint()
    {
        ...
        Assert.False(string.IsNullOrWhiteSpace(json.RootElement.GetProperty("stop_at").GetString()));
    }
